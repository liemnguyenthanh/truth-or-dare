  ...
  const startGame = useCallback(
    (config: {
      ...
      setGameState((prev) => {
        ...
        // Add participants based on config
        if (config.participants && config.participants.length > 0) {
          participants = config.participants.map((name, index) => ({
            id: `player-${index}`,
            name,
          }));
        } else if (config.gameMode === 'quick') {
          participants = [{ id: 'quick-player', name: 'Player' }];
        } else if (config.gameMode === 'spin_wheel') {
  ...
  const selectType = useCallback(
    (type: QuestionType) => {
      ...
      if (filteredQuestions.length === 0) {
        // No more questions of this type, reset to show all questions
        setQuestions(defaultQuestions);
        const allFilteredQuestions = defaultQuestions.filter(
          (q) => q.type === type && q.category === gameState.selectedCategory
        );
        if (allFilteredQuestions.length === 0) {
          // No questions found for this type and category
          return;
        }
        const randomIndex = Math.floor(
          Math.random() * allFilteredQuestions.length
        );
        const randomQuestion = allFilteredQuestions[randomIndex];
        setGameState((prev) => ({
          ...prev,
          selectedType: type,
