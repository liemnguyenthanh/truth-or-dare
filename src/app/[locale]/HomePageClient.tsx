...
export function HomePageClient({ params }: { params: { locale: string } }) {
  // Use new translation system - no manual imports needed!
  const { t } = useTranslation({
    namespaces: ['pages', 'common'],
  });
      ...
      >
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-6 auto-rows-fr'>
          {gameModeOptions.map((option, index) => {
            const isLastItem = gameModeOptions.length - 1 === index;
        ...
        <RatingModal
          ...
          description={t('modals.rating.description')}
        />
      </motion.div>
    </main>
  );
