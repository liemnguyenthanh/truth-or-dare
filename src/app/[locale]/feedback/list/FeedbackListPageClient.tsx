...
import { formatDate } from '../../../feedback/utils/formatDate';

import type { Feedback } from '@/types/feedback';

export function FeedbackListPageClient({ locale }: { locale: string }) {
  const validLocale = useMemo(() => getLocaleFromPath(`/${locale}`), [locale]);
  const { t } = useTranslation({ namespaces: ['common', 'pages'] });
  ...
  const loadFeedbacks = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(t('feedback.list.error'));
      }
    } catch (err) {
      setError(t('feedback.list.errorRetry'));
    } finally {
      setLoading(false);
    }
  }, [filters, t]);
    ...
    return (
      <div className='min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4'>
        <div className='text-center'>
          ...
          <Text>{t('feedback.list.loading')}</Text>
        </div>
      </div>
    );
  }
      ...
      <div className='max-w-6xl mx-auto mt-4'>
        ...
        >
          <div className='text-center mb-8'>
            ...
            <Text variant='large'>
              {t('feedback.list.subtitle', { total: pagination.total })}
            </Text>
          </div>
          ...
          <div className='mb-8 space-y-4'>
            <div className='flex flex-wrap gap-4'>
              <div className='flex-1 min-w-64'>
                <input
                  type='text'
                  placeholder={t('feedback.list.search')}
                  value={searchInput}
              ...
              >
                <option value='all'>{t('feedback.list.allTypes')}</option>
                <option value='bug'>{typeLabels.bug.label}</option>
                <option value='feature'>{typeLabels.feature.label}</option>
                <option value='general'>{typeLabels.general.label}</option>
                <option value='rating'>{typeLabels.rating.label}</option>
              </select>
            </div>
          </div>
          ...
          {feedbacks.length === 0 ? (
            <div className='text-center py-12'>
              ...
              <Text>{t('feedback.list.emptyDescription')}</Text>
            </div>
          ) : (
