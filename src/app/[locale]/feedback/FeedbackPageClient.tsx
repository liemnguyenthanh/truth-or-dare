...
import { FeedbackForm, FeedbackList } from '../../feedback/components';
import { useFeedbackForm, useFeedbackList } from '../../feedback/hooks';

import type { FeedbackComment } from '@/types/feedback';

export function FeedbackPageClient({ locale }: { locale: string }) {
  ...
  const initCommentData = useCallback(
    (feedbackId: number) => {
  ...
  const handleCommentSubmit = useCallback(
    (feedbackId: number) => async (e: React.FormEvent) => {
      ...
      try {
        const commentData = commentDataMap[feedbackId] || {
          content: '',
          author_name: '',
        };
        ...
        >
          ...
          <div className='border-t border-gray-200 dark:border-gray-700 pt-4 sm:pt-6'>
            <div className='flex items-center justify-between mb-3 sm:mb-4 gap-2 flex-wrap'>
              ...
              >
                {showForm ? t('feedback.hideForm') : t('feedback.submit')}
              </motion.button>
            </div>
