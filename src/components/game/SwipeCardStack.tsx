...
export default function SwipeCardStack({
  ...
  }, [currentIndex, positions.length, disabled]);
  ...
  const handleSwipeUp = useCallback(() => {
    ...
    // Then move to next
    handleSwipeRight();
  }, [positions, currentIndex, onFavorite, handleSwipeRight, disabled]);

  const handleSwipeDown = useCallback(() => {
    if (disabled) return;
    // Skip current position (same as swipe right)
    handleSwipeRight();
  }, [handleSwipeRight, disabled]);
        ...
        {cardStack.map((position, stackIndex) => {
          ...
          return (
            ...
            >
              <SwipeCard
                position={position}
                onSwipeLeft={isActive && !disabled ? handleSwipeLeft : undefined}
                onSwipeRight={isActive && !disabled ? handleSwipeRight : undefined}
                onSwipeUp={isActive && !disabled ? handleSwipeUp : undefined}
                onSwipeDown={isActive && !disabled ? handleSwipeDown : undefined}
